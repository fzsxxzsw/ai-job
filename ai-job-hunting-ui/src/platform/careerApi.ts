...
export function createCareerClient(request: Request) {
    ...
    return {
        ...
        importLegacy: async (requestId: string = crypto.randomUUID()) => request(path('/imports/legacy'), {requestId}) as Promise<{importedApplications: number; importedVersions: number; reusedApplications: number}>,
