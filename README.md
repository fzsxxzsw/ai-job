...
```text
当前 Chrome 扩展
    → Python 业务 API（9100）：登录、简历、配置、筛选、回复草稿、拒绝分析
    → 现有 MySQL：个人资料、会话、投递快照、分析报告和反馈

Python LangGraph Agent（9101）：持久任务、检查点、暂停恢复、人工审批、待执行动作
```

日常筛选与回复仍由扩展调用业务 API。Agent 的待执行动作不代表已经有浏览器执行器；收到 HR 拒绝后自动创建分析任务仍需后续业务接入。后端不自动控制 Chrome 或发送 BOSS 消息。
...
| `ai-job-agent/` | 9101 LangGraph 持久工作流基础 |
