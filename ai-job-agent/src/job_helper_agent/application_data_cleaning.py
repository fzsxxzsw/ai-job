#!/usr/bin/env python3
"""Reusable cleaning and summary pipeline for Job Helper application data.
...
Project services can call :func:`summarize_application_data` directly, while
the compatibility script delegates to this module's CLI entry point.
...
def summarize_application_data(
    *,
    user_id: int,
    container: str = "job-helper-mysql",
    mature_hours: int = 72,
    as_of: str | None = None,
    output_dir: str | Path | None = None,
    include_pii: bool = False,
    reader: DockerMySQLReader | None = None,
) -> Path:
    """Run the read-only cleaner and return the newly created report directory.

    ``reader`` is injectable so scheduled jobs and tests can reuse the same
    pipeline without depending on command-line parsing or a specific database
    transport.
    """
    if user_id <= 0:
        raise ValueError("user_id must be positive")
    if mature_hours <= 0:
        raise ValueError("mature_hours must be positive")

    database_reader = reader or DockerMySQLReader(container)
    data = load_database(database_reader, user_id)
    resumes, resume_by_hash = build_resume_versions(data, include_pii=include_pii)
    as_of_ms = parse_as_of(as_of)
    ...
    write_job_csv(report_dir / "job_details.csv", jobs)
    write_jsonl(
        report_dir / "cleaned_conversations.jsonl",
        (
            {
                "record_scope": job["record_scope"],
    ...
    write_jsonl(report_dir / "cleaned_resumes.jsonl", resumes)
...
def run(args: argparse.Namespace) -> Path:
    """Compatibility adapter for the command-line interface."""
    return summarize_application_data(
